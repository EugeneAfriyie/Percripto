      ...
      <div className=" flex flex-col sm:flex-row gap-4">
        ...
        <div className="  flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
          ...
          <div className="flex items-center gap-2 text-sm mt-1 text-gray-600">
            ...
            {/* ----- Doctor About */}
            <div className="">
                <p className=' flex items-center gap-1 text-sm font-medium text-gray-900 mr-3 '>About <img src={assets.info_icon} alt="" /></p>
      ...
      <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
        ...
        <div className="flex gap-3 w-full items-center overflow-x-scroll mt-4">
          {docSlots.length && docSlots[slotIndex].map((item,index)=>(
            <p onClick={() => setSlotTime(item.time)} className={`text-sm font-light shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'border border-gray-300 text-gray-400'}`} key={index}>
              {item.time}
            </p>
          )) }
        </div>
        <button className=' bg-primary text-white text-sm font-light px-14 py-3 rounded-full my-6 cursor-pointer'>Bok an Appointment</button>
      </div>
    </div>
  )
}
